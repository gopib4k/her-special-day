...
import { useEffect, useState, useRef, useMemo } from 'react';
import { Heart, Sparkles } from 'lucide-react';
// Notice the two dots (..) here! This tells it to go up out of the 'components' folder, then into 'assets'
...
// We set the default imageUrl to 'her' so it uses your imported image automatically!
function LoveLetter({ onNext, imageUrl = her }: LoveLetterProps) {
  const [isVisible, setIsVisible] = useState(false);
  const [displayedText, setDisplayedText] = useState('');
...
Happy Birthday once again…💜`;
  ...
  // 🌸 Lock in random values for the left side
  const leftDecorations = useMemo(() => {
    return Array.from({ length: 15 }).map((_, i) => ({
      id: `left-${i}`,
      left: `${Math.random() * 100}%`,
  ...
  return (
    ...
    >
      ...
      <div className="max-w-3xl w-full relative group z-10">
        ...
        {/* The Frosted Glass Letter Box */}
        <div className="bg-white/85 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-14 shadow-2xl border border-white/80 relative overflow-hidden transition-all duration-500 hover:shadow-purple-500/20">
          
          {/* 🖼️ Background Image Watermark (Her Image) */}
          <div className="absolute inset-0 z-0 flex items-center justify-center opacity-[0.4] pointer-events-none select-none">
            <img 
              src={imageUrl} 
              alt="Susritha Background" 
          ...
          {/* 🎯 The Reveal (After typing is complete) */}
          <div className={`transition-all duration-1000 ease-in-out overflow-hidden relative z-10 ${displayedText === fullText ? 'max-h-96 mt-12 opacity-100' : 'max-h-0 mt-0 opacity-0'}`}>
            
            {/* The Clean, Frosted "Reveal Card" */}
            <div className="flex flex-col items-center justify-center p-8 rounded-3xl bg-white/60 backdrop-blur-md border border-purple-100/50 shadow-[0_8px_32px_rgba(168,85,247,0.1)] relative">
              
              <p className="relative z-10 text-purple-600 font-semibold tracking-widest mb-6 animate-pulse text-xs md:text-sm uppercase">
                Wait… the magic isn't over yet ✨
              </p>

              {/* Magical Medallion (Grouped Hearts) */}
              <div className="relative z-10 flex items-center justify-center gap-2 mb-8 bg-white/80 px-6 py-3 rounded-full shadow-inner border border-purple-100">
                <Heart className="w-5 h-5 text-purple-500 fill-purple-400 animate-bounce" style={{ animationDelay: '0s' }} />
                <Heart className="w-7 h-7 text-pink-400 fill-pink-300 animate-bounce" style={{ animationDelay: '0.2s' }} />
                <Heart className="w-5 h-5 text-indigo-400 fill-indigo-300 animate-bounce" style={{ animationDelay: '0.4s' }} />
              </div>
